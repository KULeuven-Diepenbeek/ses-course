...
Dat betekent dat het builder-object zichzelf teruggeeft (return `this`) als resultaat van elke operatie (behalve de `build`-operatie zelf) zodat je het te bouwen object makkelijk kan configureren met opeenvolgende methode-oproepen.
Datzelfde idee wordt bijvoorbeeld ook gebruikt bij de intermediaire operaties bij streams: elke intermediaire operatie geeft opnieuw een stream terug, zodat je operaties aan elkaar kan koppelen:
```java
someStream
  .filter(...)
  .map(...)
...
Zonder een fluent interface zou het gebruik van een builder er als volgt uitzien:
```java
var builder = new Smartphone.Builder();
builder.configureStandardPhone();
builder.upgradeRam(16);
builder.upgradeStorage(128);
builder.includeGPS();
